...
export const validateCreateCompanyDto = (
  payload: unknown,
): { value?: CreateCompanyDto; error?: string } => {
  if (!isObject(payload)) {
  ...
  if (!name) {
    return { error: "name là bắt buộc." };
  }

  if (!website) {
    return { error: "website là bắt buộc." };
  }

  if (!address) {
    return { error: "address là bắt buộc." };
  }
  ...
  return {
    value: {
      ...
      ...(socialMediaLinks && socialMediaLinks.length > 0
        ? { socialMediaLinks }
        : {}),
      ...(recruitingPreferences ? { recruitingPreferences } : {}),
      ...(partnerStatusRaw
