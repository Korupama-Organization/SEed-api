...
const TEMPLATES: Record<OtpEmailPurpose, (otp: string) => { subject: string; html: string }> = {
    verify_email: (otp) => ({
        subject: '[SEeds] Mã OTP xác thực email của bạn',
        html: `
            <div style="font-family:Arial,sans-serif;max-width:480px;margin:auto;padding:32px 24px;border:1px solid #e5e7eb;border-radius:8px">
                ...
                <p>Xin chào,</p>
                <p>Bạn vừa đăng ký tài khoản trên <strong>SEeds</strong>. Vui lòng sử dụng mã OTP dưới đây để hoàn tất xác thực:</p>
    ...
    forgot_password: (otp) => ({
        subject: '[SEeds] Mã OTP đặt lại mật khẩu',
        html: `
            <div style="font-family:Arial,sans-serif;max-width:480px;margin:auto;padding:32px 24px;border:1px solid #e5e7eb;border-radius:8px">
                <h2 style="color:#dc2626;margin-bottom:8px">Đặt lại mật khẩu</h2>
                <p>Xin chào,</p>
                <p>Chúng tôi nhận được yêu cầu đặt lại mật khẩu tài khoản <strong>SEeds</strong> của bạn. Sử dụng mã OTP sau để tiếp tục:</p>
