...
const buildUitEmail = (studentID: string): string =>
  `${normalizeUserId(studentID)}@gm.uit.edu.vn`;
...
export const loginUser = async (req: Request, res: Response) => {
  try {
    const { identifier, userID, email, password } = req.body as {
      identifier?: string;
      userID?: string;
      email?: string;
      password?: string;
...
export const refreshAccessToken = async (req: Request, res: Response) => {
  try {
    ...
    const tokenValid = await deleteTempValueIfMatch(
    ...
    const passwordUpdatedAtMs =
      user.normalAuth?.passwordUpdatedAt?.getTime() ?? 0;
...
export const temporaryPasswordEncryption = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (!secret) {
      return res
        .status(500)
        .json({ message: "UIT_AUTH_SECRET is not configured" });
    }
